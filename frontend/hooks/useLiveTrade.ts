...
import { startLiveTrade, stopLiveTrade, getLiveStatus } from '@/lib/api'
...
export function useLiveTrade() {
  ...
  const start = useCallback(async (config: TradeSessionConfig) => {
    setError(null)
    try {
      const { session_id } = await startLiveTrade(config)
      setSessionId(session_id)

      const poll = setInterval(async () => {
        try {
          const data = await getLiveStatus(session_id)
          setSession(data)
