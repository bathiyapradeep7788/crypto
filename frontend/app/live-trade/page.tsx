...
export default function LiveTradePage() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [posRes, tradeRes] = await Promise.all([
        getMonitorPositions('live'), getMonitorTrades('live'),
      ])
  ...
  const doCheck = useCallback(async () => {
    ...
    try {
      const result = await checkMonitor('live', config)
      setCheckResult(result)
      setLastCheck(new Date().toLocaleTimeString())
      result.errors?.forEach((e: string) => addToast(e, 'error'))
      if (result.new_entries?.length) addToast(`${result.new_entries.length} new entry(s) opened`, 'info')
  ...
  const closePosManual = async (id: string) => {
    try { await closeMonitorPosition('live', id); addToast('Position closed', 'info'); await loadData() }
    catch (e: any) { addToast(e.message, 'error') }
  }
  const thinkPos = async (id: string) => {
    try { await thinkMonitorPosition('live', id); addToast('Marked as Think', 'info'); await loadData() }
    catch (e: any) { addToast(e.message, 'error') }
  }
    ...
    <div className="min-h-screen bg-surface">
      ...
      <main className="max-w-screen-2xl mx-auto px-6 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-bold text-white">Live Trade Monitor</h1>
            <p className="text-xs text-gray-500 mt-0.5">
              Live signal monitor — $100 per trade, one position per coin max
              <span className="ml-2 text-xs bg-red-900/40 text-red-400 border border-red-800/50 px-2 py-0.5 rounded">LIVE MODE</span>
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={monitoring ? stopMonitoring : startMonitoring}
              className={`text-sm px-4 py-2 rounded-lg font-semibold transition-all ${monitoring ? 'bg-red-700 hover:bg-red-600 text-white' : 'bg-red-600 hover:bg-red-500 text-white'}`}>
              {monitoring ? '⏹ Stop' : '▶ Start Live'}
            </button>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-1">
                {COINS.map(c => (
                  <button key={c} onClick={() => toggleCoin(c)}
                    className={`text-xs px-2 py-1 rounded transition-colors text-left ${selectedCoins.includes(c) ? 'bg-red-900/30 text-red-400 border border-red-800/40' : 'bg-surface text-gray-500 border border-surface-border hover:text-gray-300'}`}>
                    {c.replace('USDT','')}
                  </button>
                ))}
              </div>
                      ...
                      {positions.map(p => (
                        <tr key={p.id} className="border-b border-surface-border hover:bg-surface-hover">
                          ...
                          <td className="px-3 py-2">
                            <span className={`px-2 py-0.5 rounded text-xs font-semibold ${p.status==='think'?'bg-yellow-900/40 text-yellow-400':'bg-red-900/40 text-red-400'}`}>
                              {p.status}
                            </span>
                          </td>
