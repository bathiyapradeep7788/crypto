...
async def monitor_check(config: MonitorConfig):
    return await check_monitor("paper", config.model_dump())


@router.get("/monitor/positions")
async def monitor_positions():
    client = _get_client()
    res = client.table("monitor_positions").select("*").eq("mode", "paper").order("opened_at", desc=True).execute()
...
async def monitor_trades():
    client = _get_client()
    res = client.table("monitor_trades").select("*").eq("mode", "paper").order("closed_at", desc=True).limit(200).execute()
...
async def monitor_close(position_id: str):
    ...
    pos = pos_res.data[0]
    # Insert closed trade with manual exit
